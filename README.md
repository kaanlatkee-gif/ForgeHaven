...
## Custom textures (assets folder)

All sprites are baked in code, but any of them can be overridden with real
art: drop PNGs into `assets/` (next to the exe — the project copies them
there on build). Missing files fall back to the baked versions, so the
folder is entirely optional and the game runs without it.

- **Fastest start:** enable the dev menu in Settings → in-game ≡ → DEV →
  **EXPORT SPRITE TEMPLATES** — writes every current sprite as a PNG
  template to paint over (never overwrites your files). Restart to load.
- 32×32 px recommended (`train.png` may be 64×32), transparency supported.
- Names: `terrain/rock.png`, `buildings/smelter.png`, `units/raider.png`, …
  full list in `assets/README.txt`. Belts/rails are directional
  (`belt_right.png`…) or supply one `belt.png` pointing right and the
  other facings are rotated automatically.
