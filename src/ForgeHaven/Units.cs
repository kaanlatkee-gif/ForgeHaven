...
// ---------------------------------------------------------------------------
//  MADDOG iter-9: trade caravan (G13). Walks in, parks by the hub, opens a
//  barter window, packs up and leaves. Deals live in Bal.TradeDeals.
// ---------------------------------------------------------------------------
...
public sealed class Trader
{
    ...
    public List<Point>? Path;
    public int PathIdx;
    public bool Arrived;             // parked and trading
    public bool Leaving;             // window over, heading out
    public float WindowT;            // seconds of trade window left
    public int[] SoldDeals = new int[Bal.TradeDeals.Length];
    public float WalkPhase, FaceAngle;
    public bool Dead;                // left the map - Game removes

    /// <summary>Park spot: a few tiles south-east of the hub ring.</summary>
    public static (int x, int y) ParkSpot(Game g)
    {
        var hc = g.HubRef.CenterTile;
        int hcx = (int)hc.X, hcy = (int)hc.Y;
        for (int r = 4; r < 10; r++)
            for (int a = 0; a < 12; a++)
            {
                float ang = a / 12f * MathF.PI * 2f;
                int x = hcx + (int)(MathF.Cos(ang) * r), y = hcy + (int)(MathF.Sin(ang) * r);
                if (g.World.InBounds(x, y) && g.World.WalkEnemy(x, y)) return (x, y);
            }
        return (hcx, hcy + 4);
    }
    ...
    public void Update(Game g, float dt)
    {
        ...
        if (!Arrived && !Leaving && (Path == null || PathIdx >= Path.Count))
        {
            Arrived = true;
            WindowT = Bal.TradeWindowSec;
            g.AddLog(Loc.T("The trade caravan has parked - click it to barter."), Pal.Accent);
        }
        ...
        if (Arrived)
        {
            ...
            if (WindowT <= 0)
            {
                Arrived = false;
                Leaving = true;
                var (ex, ey) = ParkSpot(g);      // head back out the way it came
                Path = g.World.FindPath((int)PosX, (int)PosY, ex + 30, ey + 30, enemy: false);
                PathIdx = 0;
                g.AddLog(Loc.T("The caravan packs up and moves on."), Pal.TextDim);
            }
            return;
        }
        ...
        if (Path != null && PathIdx < Path.Count)
        {
            ...
            float dx = txp - PosX, dy = typ - PosY;
            float step = MathF.Sqrt(dx * dx + dy * dy);
            float spd = Bal.TraderSpeed * dt;
            FaceAngle = MathF.Atan2(dy, dx);
            if (step <= spd) { PosX = txp; PosY = typ; PathIdx++; }
            else { PosX += dx / step * spd; PosY += dy / step * spd; }
        }
        else if (Leaving) Dead = true;           // path done (or blocked): gone
    }
}
